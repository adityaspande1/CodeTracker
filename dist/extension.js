...
function activate(context) {
    ...
    const isAuthenticated = context.globalState.get(storageKey, false);
    const disposable1 = vscode.commands.registerCommand('codetracker.authenticate', async () => {
        if (!isAuthenticated) {
            const userResponse = await vscode.window.showInformationMessage('Allow WorkSpace-Tracker to Authenticate with GitHub', 'Yes', 'No');
            if (userResponse === 'Yes') {
                try {
...
class WorkSpaceTracker extends events_1.EventEmitter {
    ...
    setupWatcher() {
        ...
        this.watcher.onDidCreate((uri) => {
            this.trackChange(uri, 'created');
            console.log(`🟢 File Created: ${uri.fsPath}`);
        });
        this.watcher.onDidChange((uri) => {
            this.trackChange(uri, 'modified');
            console.log(`🟡 File Modified: ${uri.fsPath}`);
        });
        this.watcher.onDidDelete((uri) => {
            this.trackChange(uri, 'deleted');
